...
const hasAlternanceHours = computed(() => {
    ...
    const hasTeacherAlternance =
        resourceSheetDTO.value?.hoursTeacherAlternance != null &&
    ...
    const hasLocalAlternance =
        (localHoursAlternance.value.cm && localHoursAlternance.value.cm > 0) ||
...
const localHoursTotal = computed(() => {
    const cm =
        typeof localHours.value.cm === 'number'
            ? localHours.value.cm
            : parseFloat(localHours.value.cm) || 0
    const td =
        typeof localHours.value.td === 'number'
            ? localHours.value.td
            : parseFloat(localHours.value.td) || 0
    const tp =
        typeof localHours.value.tp === 'number'
            ? localHours.value.tp
            : parseFloat(localHours.value.tp) || 0
    return cm + td + tp
})

const localHoursAlternanceTotal = computed(() => {
    const cm =
        typeof localHoursAlternance.value.cm === 'number'
            ? localHoursAlternance.value.cm
            : parseFloat(localHoursAlternance.value.cm) || 0
    const td =
        typeof localHoursAlternance.value.td === 'number'
            ? localHoursAlternance.value.td
            : parseFloat(localHoursAlternance.value.td) || 0
    const tp =
        typeof localHoursAlternance.value.tp === 'number'
            ? localHoursAlternance.value.tp
            : parseFloat(localHoursAlternance.value.tp) || 0
    return cm + td + tp
})
...
const dbHoursTotal = computed(() => {
    ...
    return (
        (hoursPerStudent.value.cm || 0) +
        (hoursPerStudent.value.td || 0) +
        (hoursPerStudent.value.tp || 0)
    )
})
...
const dbHoursTotalAlternance = computed(() => {
    ...
    return (
        (hoursPerStudentAlternance.value.cm || 0) +
        (hoursPerStudentAlternance.value.td || 0) +
        (hoursPerStudentAlternance.value.tp || 0)
    )
})
...
const onDrop = (type, dropIndex) => {
    if (draggedType.value === type && draggedItem.value !== null) {
        ...
        // Update panel height
        nextTick(() => {
            const pedagogicalPanel = document
                .querySelector('.pedagogical-content')
                ?.closest('.panel')
            if (pedagogicalPanel) {
                updatePanelHeight(pedagogicalPanel)
            }
...
const saveResourceSheet = async () => {
    ...
    const validSkills = localSkills.value.filter(
        (skill) =>
            skill.label &&
            skill.label.trim() !== '' &&
            skill.description &&
    ...
    const hasValidPedagogicalContent =
        localPedagogicalContent.value.cm.some(
            (item) => item.content && item.content.trim() !== '',
        ) ||
        localPedagogicalContent.value.td.some(
            (item) => item.content && item.content.trim() !== '',
        ) ||
        localPedagogicalContent.value.tp.some(
            (item) => item.content && item.content.trim() !== '',
        ) ||
    ...
    try {
        ...
        const updateDTO = {
            ...
            teacherHoursAlternance: hasAlternanceHours.value
                ...
                : null,
            pedagogicalContent: {
                cm: localPedagogicalContent.value.cm.map((item) => ({
                    order: item.number,
                    content: item.content,
                })),
                td: localPedagogicalContent.value.td.map((item) => ({
                    order: item.number,
                    content: item.content,
                })),
                tp: localPedagogicalContent.value.tp.map((item) => ({
                    order: item.number,
                    content: item.content,
                })),
                ds: localPedagogicalContent.value.ds.map((item) => ({
                    order: item.number,
                    content: item.content,
                })),
            },
...
onMounted(async () => {
    ...
    if (resourceSheetId.value) {
        try {
            ...
            if (resourceSheetDTO.value.keywords && resourceSheetDTO.value.keywords.length > 0) {
                localKeywords.value = resourceSheetDTO.value.keywords.map((kw) => ({
                    keyword: kw,
                    isNew: false,
                }))
            } else {
            ...
            if (resourceSheetDTO.value.modalities && resourceSheetDTO.value.modalities.length > 0) {
                localModalities.value = resourceSheetDTO.value.modalities.map((mod) => ({
                    modality: mod,
                    isNew: false,
                }))
            } else {
            ...
            if (resourceSheetDTO.value.hoursTeacher) {
                ...
                localHours.value = {
                    cm:
                        resourceSheetDTO.value.hoursTeacher.cm !== null &&
                        resourceSheetDTO.value.hoursTeacher.cm !== undefined
                            ? resourceSheetDTO.value.hoursTeacher.cm
                            : '',
                    td:
                        resourceSheetDTO.value.hoursTeacher.td !== null &&
                        resourceSheetDTO.value.hoursTeacher.td !== undefined
                            ? resourceSheetDTO.value.hoursTeacher.td
                            : '',
                    tp:
                        resourceSheetDTO.value.hoursTeacher.tp !== null &&
                        resourceSheetDTO.value.hoursTeacher.tp !== undefined
            ...
            // Hours Alternance
            console.log(
                '📊 hoursTeacherAlternance from DTO:',
                resourceSheetDTO.value.hoursTeacherAlternance,
            )
            ...
            if (resourceSheetDTO.value.hoursTeacherAlternance) {
                ...
                localHoursAlternance.value = {
                    cm:
                        resourceSheetDTO.value.hoursTeacherAlternance.cm !== null &&
                        resourceSheetDTO.value.hoursTeacherAlternance.cm !== undefined
                            ? resourceSheetDTO.value.hoursTeacherAlternance.cm
                            : '',
                    td:
                        resourceSheetDTO.value.hoursTeacherAlternance.td !== null &&
                        resourceSheetDTO.value.hoursTeacherAlternance.td !== undefined
                            ? resourceSheetDTO.value.hoursTeacherAlternance.td
                            : '',
                    tp:
                        resourceSheetDTO.value.hoursTeacherAlternance.tp !== null &&
                        resourceSheetDTO.value.hoursTeacherAlternance.tp !== undefined
            ...
            if (resourceSheetDTO.value.pedagogicalContent) {
                ...
                localPedagogicalContent.value = {
                    cm: (content.cm || []).map((item) => ({
                        number: item.order,
                        content: item.content,
                    })),
                    td: (content.td || []).map((item) => ({
                        number: item.order,
                        content: item.content,
                    })),
                    tp: (content.tp || []).map((item) => ({
                        number: item.order,
                        content: item.content,
                    })),
                    ds: (content.ds || []).map((item) => ({
                        number: item.order,
                        content: item.content,
                    })),
                }
            ...
            if (resourceSheetDTO.value.tracking) {
                localResourceTracking.value = {
                    ...
                    improvementSuggestions:
                        resourceSheetDTO.value.tracking.improvementSuggestions || '',
                }
            }
    ...
    const textareas = document.querySelectorAll(
        '#text_area_styled, .auto-resize-textarea, .skill-input-description, .modality-textarea',
    )
    textareas.forEach((textarea) => {
        const autoResize = () => {
            textarea.style.height = 'auto'
        ...
        <div id="background_Form">
                        ...
                        <div v-for="(skill, index) in localSkills" :key="index" class="skill-row">
                            <div class="skill-inputs">
                                <input
                                    type="text"
                                    v-model="skill.label"
                                    placeholder="Label de la compétence..."
                                    class="skill-input skill-input-label"
                                />
                                <textarea
                                    v-model="skill.description"
                                    placeholder="Description de la compétence..."
                                    class="skill-input skill-input-description"
                                    rows="2"
                                ></textarea>
                            </div>
                            <button
                                ...
                                :disabled="localSkills.length === 1"
                                title="Supprimer cette compétence"
                            >
                                ✕ Supprimer
                            </button>
                        </div>
                        <button @click="addSkillRow" class="btn-add-skill">
                            + Ajouter une compétence
                        </button>
                    </div>
                    <span v-if="validationErrors.skills" class="error-message"
                        >Merci de remplir au moins une compétence avec un label et une
                        description</span
                    >
                </div>
            </div>
                ...
                <div v-else class="sae_switches_container">
                    <div v-for="sae in saeList" :key="sae.id" class="sae_switch_item">
                        <label class="switch">
                            <input
                                type="checkbox"
                                :checked="isSaeLinked(sae.id)"
                                @change="toggleSaeLink(sae.id)"
                            />
                            <span class="slider"></span>
                        </label>
                        <span
                            class="sae_label"
                            :title="`ID: ${sae.id}, ApogeeCode: ${sae.apogeeCode}, Label: ${sae.label}`"
                            >{{ sae.label || 'SAE sans nom' }}</span
                        >
                    </div>
                </div>
            </div>
                    ...
                    <div class="keywords-container">
                        ...
                        >
                            <input
                                ...
                                @keydown.enter="handleKeywordEnter(index)"
                                placeholder="Mot-clé..."
                                class="keyword-input"
                            />
                            <button
                        ...
                        <button @click="addKeyword" class="btn-add-keyword">
                            + Ajouter un mot-clé
                        </button>
                    </div>
                </div>
                    ...
                    <div class="modalities-list">
                        ...
                        >
                            <textarea
                                ...
                                placeholder="Modalité de mise en œuvre..."
                                class="modality-textarea"
                                rows="3"
                            ></textarea>
                            <button
                                @click="removeModality(index)"
                                class="btn-remove-modality"
                                title="Supprimer cette modalité"
                            >
                                ✕
                            </button>
                        </div>
                        <button @click="addModality" class="btn-add-modality">
            ...
            <div id="hours_section">
                ...
                <p
                    ...
                    style="color: white; margin-top: 1.5vw; margin-bottom: 0.5vw"
                >
                    Formation Initiale
                </p>
                <div class="hours_container">
                    <div class="hours_row">
                        ...
                        <div class="hours_total_display">
                            <p class="hours_total_text">
                                Nombre d'heures total :
                                <span class="hours_total_value">{{ localHoursTotal }}</span> /
                                <span class="hours_total_value">{{ dbHoursTotal }}</span>
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="hours_container">
                        <div class="hours_row">
                            ...
                            <div class="hours_total_display">
                                <p class="hours_total_text">
                                    Nombre d'heures total :
                                    <span class="hours_total_value">{{
                                        localHoursAlternanceTotal
                                    }}</span>
                                    /
                                    <span class="hours_total_value">{{
                                        dbHoursTotalAlternance
                                    }}</span>
                                </p>
                            </div>
                        </div>
            ...
            <div id="pedagogical_content_section">
                ...
                <div class="pedagogical-content">
                    ...
                    <div class="pedagogical-section">
                        ...
                        <p class="pedagogical-subtitle">
                            Détailler ici le contenu pédagogique des CM
                        </p>
                        <div class="pedagogical-items-container">
                            <div
                                v-if="localPedagogicalContent.cm.length === 0"
                            ...
                            >
                                ...
                                ></textarea>
                                <button
                                    @click="removePedagogicalItem('cm', index)"
                                    class="btn-remove-pedagogical"
                                    title="Supprimer"
                                >
                                    ✕
                                </button>
                        ...
                        <div class="pedagogical-footer">
                            <button @click="addPedagogicalItem('cm')" class="btn-add-pedagogical">
                                + Ajouter
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="pedagogical-section">
                        ...
                        <p class="pedagogical-subtitle">
                            Détailler ici le contenu pédagogique des TD
                        </p>

                        <div class="pedagogical-items-container">
                            <div
                                v-if="localPedagogicalContent.td.length === 0"
                                class="no-content-message"
                            >
                                Aucun contenu TD
                            </div>
                            ...
                            >
                                ...
                                ></textarea>
                                <button
                                    @click="removePedagogicalItem('td', index)"
                                    class="btn-remove-pedagogical"
                                    title="Supprimer"
                                >
                                    ✕
                        ...
                        <div class="pedagogical-footer">
                            <button @click="addPedagogicalItem('td')" class="btn-add-pedagogical">
                                + Ajouter
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="pedagogical-section">
                        ...
                        <p class="pedagogical-subtitle">
                            Détailler ici le contenu pédagogique des TP
                        </p>
                        <div class="pedagogical-items-container">
                            <div
                                v-if="localPedagogicalContent.tp.length === 0"
                                class="no-content-message"
                            >
                                Aucun contenu TP
                            </div>
                            ...
                            >
                                ...
                                ></textarea>
                                <button
                                    @click="removePedagogicalItem('tp', index)"
                                    class="btn-remove-pedagogical"
                                    title="Supprimer"
                                >
                                    ✕
                        ...
                        <div class="pedagogical-footer">
                            <button @click="addPedagogicalItem('tp')" class="btn-add-pedagogical">
                                + Ajouter
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="pedagogical-section">
                        ...
                        <p class="pedagogical-subtitle">
                            Détailler ici le contenu pédagogique des DS
                        </p>
                        <div class="pedagogical-items-container">
                            <div
                                v-if="localPedagogicalContent.ds.length === 0"
                                class="no-content-message"
                            >
                                Aucun contenu DS
                            </div>
                            ...
                            >
                                ...
                                ></textarea>
                                <button
                                    @click="removePedagogicalItem('ds', index)"
                                    class="btn-remove-pedagogical"
                                    title="Supprimer"
                                >
                                    ✕
                        ...
                        <div class="pedagogical-footer">
                            <button @click="addPedagogicalItem('ds')" class="btn-add-pedagogical">
                                + Ajouter
                            </button>
                        </div>
                    </div>
                </div>
                <span v-if="validationErrors.pedagogicalContent" class="error-message"
                    >Merci de remplir au moins un contenu dans une des colonnes (CM, TD, TP ou
                    DS)</span
                >
            </div>
            <div id="form">
